...
class WorkerResourceConfig(BaseModel):
    ...
    requests: dict[str, str] = Field(default_factory=lambda: {"cpu": "100m", "memory": "256Mi"})
...
class AdaptiveConfig(ConfigModel):
    ...
    central_store: CentralStoreConfig | None = Field(default_factory=CentralStoreConfig)
    workers: WorkersConfig | None = Field(default_factory=WorkersConfig)
    ...
    def get_central_store_config(self) -> dict[str, Any] | None:
