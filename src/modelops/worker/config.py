...
class RuntimeConfig:
    ...
    bundle_registry: str | None = None  # e.g., "ghcr.io/org/models"
    bundles_cache_dir: str = "/tmp/modelops/bundles"
    bundles_dir: str | None = None  # For file source
    ...
    azure_connection_string: str | None = None  # Optional explicit connection string
    ...
    def from_env(cls) -> "RuntimeConfig":
        ...
        config.bundles_cache_dir = os.environ.get(
            "MODELOPS_BUNDLES_CACHE_DIR", config.bundles_cache_dir
        )
        config.bundles_dir = os.environ.get("MODELOPS_BUNDLES_DIR", config.bundles_dir)
        config.bundle_insecure = (
            os.environ.get("MODELOPS_BUNDLE_INSECURE", "false").lower() == "true"
        )
        ...
        config.azure_connection_string = os.environ.get(
        ...
        config.max_warm_processes = int(
            os.environ.get("MODELOPS_MAX_WARM_PROCESSES", config.max_warm_processes)
        )
        ...
        # Process pool configuration
        config.force_fresh_venv = (
            os.environ.get("MODELOPS_FORCE_FRESH_VENV", "false").lower() == "true"
        )
        config.validate_deps_on_reuse = (
            os.environ.get("MODELOPS_VALIDATE_DEPS", "true").lower() == "true"
        )
        config.max_process_reuse_count = int(
            os.environ.get("MODELOPS_MAX_PROCESS_REUSE", config.max_process_reuse_count)
        )
        config.process_ttl_seconds = int(
            os.environ.get("MODELOPS_PROCESS_TTL", config.process_ttl_seconds)
        )
