...
class FileBundleRepository:
    ...
    def ensure_local(self, bundle_ref: str) -> tuple[str, Path]:
        ...
        if bundle_ref.startswith("sha256:"):
            ...
            raise ValueError(
                f"Bundle with digest {digest[:12]} not found in cache or local bundles"
            )
        ...
        if bundle_ref == "local://dev":
            ...
            source_path = (
                Path(bundle_ref) if bundle_ref.startswith("/") else self.bundles_dir / bundle_ref
            )
        ...
        if not source_path.exists():
            raise ValueError(f"Bundle not found: {bundle_ref} (resolved to {source_path})")
