...
def atomic_write(path: Union[str, Path], content: bytes) -> None:
...
def safe_read(path: Union[str, Path]) -> Optional[bytes]:
...
def atomic_rename(src: Union[str, Path], dst: Union[str, Path]) -> None:
