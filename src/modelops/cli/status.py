...
from .display import commands, console, error, info, section, success, warning
from .utils import resolve_env

app = typer.Typer(help="Infrastructure status and health checks")


def get_all_stacks(env: str | None = None) -> dict[str, list[dict[str, Any]]]:
                        ...
                        try:
                            ...
                            if component == "adaptive" and parsed.get("run_id"):
                                outputs = automation.outputs(
                                    ...
                                    parsed["run_id"],
                                    refresh=False,
                                    work_dir=str(work_dir / parsed["run_id"]),
                                )
                            else:
                                outputs = automation.outputs(
                                    component,
...
def status_all(
    ...
    smoke_test: bool = typer.Option(
        False, "--smoke-test", help="Run smoke tests for connectivity validation"
    ),
):
            ...
            if outputs and status == "✓ Deployed":
                if component == "infra":
                    ...
                    if containers:
                        ...
                        if container_names:
                            console.print(
                                f"    [dim]• Containers:[/dim] {', '.join(container_names[:4])}"
                            )
                    ...
                    if replicas:
                        console.print(
                            f"    [dim]• Worker replicas:[/dim] [green]{replicas}[/green]"
                        )
                    if outputs.get("postgres_dsn"):
                        console.print("    [dim]• Database:[/dim] [green]✓ Postgres[/green]")
    ...
    commands(
        [
            (
                "Workspace details",
                f"mops workspace status{' --env ' + env_filter if env_filter else ''}",
            ),
            (
                "Storage info",
                f"mops storage info{' --env ' + env_filter if env_filter else ''}",
            ),
        ]
    )
            ...
            if namespace:
                ...
                try:
                    ...
                    if result.returncode == 0 and "secret/modelops-storage" in result.stdout:
                        ...
                        if (
                            keys_result.returncode == 0
                            and "AZURE_STORAGE_CONNECTION_STRING" in keys_result.stdout
                        ):
        ...
        for ws_stack in all_stacks.get("workspace", []):
            ...
            if namespace:
                ...
                try:
                    ...
                    if scheduler_pods:
                        running = all(
                            pod["phase"] == "Running" and pod["ready"] for pod in scheduler_pods
                        )
                        if running:
                            success("  ✓ Dask scheduler running")
                        else:
                            warning("  ⚠ Dask scheduler not ready")
                    else:
