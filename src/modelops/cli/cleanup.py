...
from ..core.paths import WORK_DIRS, ensure_work_dir
from ..core.subprocess_utils import run_pulumi_command
from .common_options import env_option, yes_option
from .display import commands, dim, error, info, section, success, warning
...
def unreachable(
    ...
    try:
        ...
        result = run_pulumi_command(
            cmd, cwd=str(work_dir), env={"PULUMI_K8S_DELETE_UNREACHABLE": "true"}
        )
...
@app.command()
def all(env: str | None = env_option(), yes: bool = yes_option()):
    ...
    if errors:
        ...
        commands([("Manual cleanup", "mops cleanup unreachable <component>")])


@app.command()
def orphaned(env: str | None = env_option(), yes: bool = yes_option()):
