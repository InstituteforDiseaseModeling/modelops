...
@app.command()
def create(
    name: str = typer.Option("modelops-registry", "--name", "-n", help="Registry name"),
    provider: str | None = typer.Option(
        None, "--provider", "-p", help="Registry provider (azure, dockerhub, ghcr)"
    ),
    config: Path | None = typer.Option(
        None, "--config", "-c", help="Registry configuration file (YAML)"
    ),
    ...
    # Load configuration
    registry_config = {"provider": provider, "environment": env}
    ...
    if provider == "azure":
        ...
        subscription_id = registry_config.get("subscription_id") or None  # Will use Azure CLI auth
    ...
    try:
        ...
        if provider == "azure":
            ...
            section("\nLogin to registry:")
            commands([("", f"az acr login --name {registry_name}")])
            section("\nBuild and push images:")
            commands(
                [
                    (
                        "Build",
                        f"docker build -t {registry_name}.azurecr.io/dask-worker:latest .",
                    ),
                    (
                        "Push",
                        f"docker push {registry_name}.azurecr.io/dask-worker:latest",
                    ),
                ]
            )
        ...
        handle_pulumi_error(
            e,
            "~/.modelops/pulumi/registry",
            StackNaming.get_stack_name("registry", env),
        )
        raise typer.Exit(1)


@app.command()
def destroy(env: str | None = env_option(), yes: bool = yes_option()):
        ...
        if provider == "azure":
            ...
            commands(
                [
                    ...
                    (
                        "Show tags",
                        f"az acr repository show-tags --name {registry_name} --repository IMAGE",
                    ),
                ]
            )
...
def env(
    env: str | None = env_option(),
    format: str = typer.Option("bash", "--format", "-f", help="Output format (bash, json, make)"),
):
...
@app.command()
def login(env: str | None = env_option()):
