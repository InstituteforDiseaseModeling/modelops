...
from .common_options import config_option, env_option, yes_option
from .display import (
    commands,
    console,
    error,
...
def down(
    ...
    force: bool = typer.Option(
        False, "--force", help="Skip dependency checks and force destruction"
    ),
    yes: bool = yes_option(),
):
...
def status(
    ...
    provider: str | None = typer.Option(
        None, "--provider", "-p", help="Cloud provider (azure, aws, gcp)"
    ),
):
...
def kubeconfig(
    ...
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Write kubeconfig to file instead of stdout"
    ),
    merge: bool = typer.Option(False, "--merge", help="Merge with existing ~/.kube/config"),
):
