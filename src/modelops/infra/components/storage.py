...
class BlobStorage(pulumi.ComponentResource):
    ...
    def __init__(
        ...
        account = self._create_storage_account(
            name, env, location, rg_name, username, desired_account_name
        )
        ...
        sas_connection_string = self._create_sas_connection_string(
            account, rg_name, storage_config.containers
        )

        # Create K8s secret if infra reference provided
        if infra_stack_ref:
            self._create_k8s_secret(infra_stack_ref, account.name, connection_string, env)
    ...
    def _create_storage_account(
        ...
        rg_name: str,
        username: str,
        desired_account_name: str | None = None,
    ):
        ...
        return azure.storage.StorageAccount(
            ...
            tags={"managed-by": "modelops", "component": "storage", "environment": env},
            opts=pulumi.ResourceOptions(parent=self),
        )

    def _create_containers(
        self, account: azure.storage.StorageAccount, containers: list, rg_name: str
    ):
    ...
    def _get_connection_string(self, account: azure.storage.StorageAccount, rg_name: str):
        ...
        keys = azure.storage.list_storage_account_keys_output(
            resource_group_name=rg_name, account_name=account.name
        )

        return pulumi.Output.all(account.name, keys.keys[0].value).apply(
            lambda args: (
    ...
    def _create_sas_connection_string(
        self, account: azure.storage.StorageAccount, rg_name: str, containers: list
    ):
        ...
        sas = azure.storage.list_storage_account_service_sas_output(
            ...
            canonicalized_resource=pulumi.Output.concat(
                "/blob/", account.name, "/", bundles_container
            ),
        ...
        return pulumi.Output.all(account.name, sas.service_sas_token).apply(
            lambda args: (
    ...
    def _create_k8s_secret(
        ...
        k8s.core.v1.Secret(
            "storage-secret-default",
            metadata=k8s.meta.v1.ObjectMetaArgs(name="modelops-storage", namespace="default"),
            string_data={
